...
test('should be possible to save app configuration', async ({ appConfigPage, page }) => {
  ...
  await page.getByRole('textbox', { name: 'Agent endpoint URL' }).clear();
  await page.getByRole('textbox', { name: 'Agent endpoint URL' }).fill('http://www.my-awsome-grafana-app.com/api');
